...
interface LandingPageProps {
	...
	onSelectStory: (
		view: "ceo" | "finance" | "gtm" | "hr" | "compare",
		cardId: string,
	) => void;
...
export const LandingPage: React.FC<LandingPageProps> = ({
	...
	const filteredSearchOptions = searchOptions.filter((opt) => {
		...
		return (
			opt.query.toLowerCase().includes(queryLower) ||
				...
				<div className="bg-panel border border-panel-border rounded-2xl p-4.5 shadow-sm space-y-3.5 h-full">
					<div className="space-y-1 pl-1">
						...
						<p className="text-[9px] text-ink-soft leading-normal font-light">
							Click a shortcut below to navigate directly to target dashboard
							elements with highlighting.
						</p>
					</div>
					...
					<div className="flex flex-col gap-2.5">
						{userStories.map((story, sIdx) => (
							<button
								...
								<ChevronRight
									size={12}
									className="text-accent shrink-0 mt-0.5 group-hover:translate-x-0.5 transition-transform"
								/>
			...
			<div className="flex-1 flex flex-col justify-start items-center space-y-6">
				...
				<div className="text-center max-w-2xl space-y-2">
					<div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-accent/10 border border-accent/25 text-[9px] font-black uppercase text-accent tracking-widest leading-none">
						<Sparkles size={10} className="animate-pulse" /> Unified Executive
						Suite
					</div>
					...
					<p className="text-xs text-ink-soft font-light leading-relaxed">
						Select an executive persona view or search across consolidated
						insights using our AI-assisted visualization engine.
					</p>
				</div>
				...
				<div className="w-full max-w-xl relative space-y-3">
					...
					{showSearchResults && (
						<div className="absolute top-full left-0 right-0 mt-2 bg-panel border border-panel-border rounded-2xl shadow-lg z-50 overflow-hidden divide-y divide-panel-border">
							<div className="bg-panel-2 px-4 py-2 flex justify-between items-center">
								<span className="text-[8px] font-bold text-ink-soft uppercase tracking-wider">
							...
							{filteredSearchOptions.length > 0 ? (
								...
								<div className="p-4 text-center text-[10px] text-ink-soft">
									No matching AI queries found. Try searching for "roadmap",
									"capex", or "spirit".
								</div>
							)}
						</div>
					...
					<div className="flex flex-wrap items-center justify-center gap-2 max-w-xl">
						<span className="text-[8px] font-black text-ink-soft uppercase tracking-wider mr-1">
							Suggestions:
						</span>
										...
										{(dash.id as string) === "custom" && (
											<button
												onClick={(e) => {
													...
													if (
														confirm(
															"Are you sure you want to delete your custom dashboard? This will remove all selected visuals.",
														)
													) {
														onDeleteCustomDashboard?.();
													}
								...
								<div className="grid grid-cols-3 gap-1 text-center">
									{dash.kpiPreview.map((kpi, kIdx) => (
										<div key={kIdx} className="space-y-0.5">
											...
											<span
												className={`text-[11px] font-black tracking-tight block leading-none ${
													kpi.status === "red"
														? "text-rag-red"
														: kpi.status === "amber"
															? "text-rag-amber"
