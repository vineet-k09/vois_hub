...
const Transformation: React.FC<TransformationProps> = ({ onDrillDown }) => {
	...
	return (
		<section
			...
			className="bg-panel border border-panel-border rounded-2xl p-4.5 shadow-sm relative 
			...
			<div>
				...
				<div className="grid grid-cols-1 gap-2.5 pl-1">
					{req06.roadmap?.map((item: any) => {
						...
						const containerSizeClass = isRed
							? "py-4.5 px-4 shadow-xs"
							: isAmber
								? "py-3.5 px-3.5"
								: "py-3 px-3";
						...
						if (isRed) {
							cardStyleClass =
								"bg-rag-red/10 border-rag-red/20 hover:bg-rag-red/15 text-ink ring-1 ring-rag-red/5";
						} else if (isAmber) {
							cardStyleClass =
								"bg-rag-amber/10 border-rag-amber/20 hover:bg-rag-amber/15 text-ink ring-1 ring-rag-amber/5";
						} else {
							cardStyleClass =
								"bg-panel-2/30 border-panel-border hover:bg-panel-2 hover:border-accent/40 text-ink";
						}

						const getProgramTooltip = (
							name: string,
							prog: number,
							details: any,
						) => {
						...
						return (
							<Tooltip
								key={item.name}
								content={getProgramTooltip(item.name, item.progress, parsed)}
								position="top">
								<div
									...
									<div className="lg:col-start-1 lg:row-start-1 min-w-0 flex items-center gap-1.5">
										...
										<h4
											className={`${nameFontClass} font-bold text-ink group-hover:text-accent transition-colors `}>
											{item.name}
										</h4>
									</div>
									...
									<div className="lg:col-start-2 lg:row-start-1 lg:row-span-2 flex flex-col gap-2 w-full min-w-0">
										...
										<div className="flex justify-between items-center gap-3 leading-none">
											<div className="flex items-center gap-2 min-w-0">
												<span className="text-[10px] text-rag-green font-bold shrink-0">
													{parsed.value}
												</span>
												<span className="text-[8.5px] text-ink-soft truncate hidden sm:inline">
													• {parsed.alignment}
												</span>
											</div>
											<span
									...
									<div className="lg:col-start-1 lg:row-start-2 min-w-0 mt-0.5 lg:mt-0">
										<p
											className={`${ownerFontClass} text-ink-soft font-medium tracking-wide leading-none`}>
											Owner:{" "}
