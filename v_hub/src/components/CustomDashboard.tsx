...
import { ArrowUp, ArrowDown, Trash2, Plus, LayoutGrid, X, Edit3 } from 'lucide-react';
...
export const AVAILABLE_VISUALS: VisualDefinition[] = [
  // CEO View
  { id: 'ceo-kpis', name: 'Critical KPI Grid', category: 'CEO Strategy', dashboard: 'ceo' },
  { id: 'ceo-summary', name: 'AI Executive Summary & Watchpoints', category: 'CEO Strategy', dashboard: 'ceo' },
  { id: 'ceo-transformation', name: 'Group Transformation Roadmap', category: 'CEO Strategy', dashboard: 'ceo' },
  { id: 'ceo-gmt', name: 'GMT Owner Grip / Exception Details', category: 'CEO Strategy', dashboard: 'ceo' },
  { id: 'ceo-goals', name: 'FY27 Strategic Goals Alignment', category: 'CEO Strategy', dashboard: 'ceo' },
  { id: 'ceo-exceptions', name: 'Operating Exceptions (Helios, Atlas, Iris)', category: 'CEO Strategy', dashboard: 'ceo' },
  { id: 'ceo-customer', name: 'Customer Account Performance Lens', category: 'CEO Strategy', dashboard: 'ceo' },
  
  // Finance View
  { id: 'finance-kpis', name: 'Finance KPI Grid (EBITDA, Opex, Capex)', category: 'Finance Portfolio', dashboard: 'finance' },
  { id: 'finance-risk', name: 'Cross-Functional Risk Heatmap & List', category: 'Finance Portfolio', dashboard: 'finance' },
  { id: 'finance-pq', name: 'PxQ / Billing Integration Summary', category: 'Finance Portfolio', dashboard: 'finance' },
  { id: 'finance-gov', name: 'Basic Reporting Reliability (Parked)', category: 'Finance Portfolio', dashboard: 'finance' },
  
  // GTM View
  { id: 'gtm-kpis', name: 'GTM KPI Grid (Win Conv, Pipeline, Bookings)', category: 'GTM Commercial', dashboard: 'gtm' },
  { id: 'gtm-pipeline', name: 'GTM Pipeline Funnel & Stage Metrics', category: 'GTM Commercial', dashboard: 'gtm' },
  { id: 'gtm-revrec', name: 'GTM Revenue Recognition & Target Progress', category: 'GTM Commercial', dashboard: 'gtm' },
  { id: 'gtm-delivery', name: 'GTM Top-10 Delivery Risk Dashboard', category: 'GTM Commercial', dashboard: 'gtm' },
  { id: 'gtm-offerings', name: 'GTM Service Offerings Performance Grid', category: 'GTM Commercial', dashboard: 'gtm' },
  { id: 'gtm-wallet', name: 'GTM Share of Wallet vs Competitor Performance', category: 'GTM Commercial', dashboard: 'gtm' },
  { id: 'gtm-customers', name: 'GTM Customer Accounts SLA & NPS Tracker', category: 'GTM Commercial', dashboard: 'gtm' },
  { id: 'gtm-competitive', name: 'GTM Competitive Briefing Dashboard', category: 'GTM Commercial', dashboard: 'gtm' },
  
  // HR View
  { id: 'hr-kpis', name: 'HR KPI Grid (FTEs, Attrition, Cost)', category: 'HR Strategic', dashboard: 'hr' },
  { id: 'hr-cross', name: 'Cross-Functional HR Headcount Mastership', category: 'HR Strategic', dashboard: 'hr' },
  { id: 'hr-promotions', name: 'Talent Stability & Promotions Glide Path', category: 'HR Strategic', dashboard: 'hr' },
  { id: 'hr-headcount', name: 'Headcount & Billing Efficiency Metrics', category: 'HR Strategic', dashboard: 'hr' },
  { id: 'hr-demand', name: 'Demand Outlook & Fulfillment Funnel', category: 'HR Strategic', dashboard: 'hr' },
  { id: 'hr-spirit', name: 'Spirit Beat Drivers & Employee Sentiment', category: 'HR Strategic', dashboard: 'hr' },
  { id: 'hr-ld', name: 'L&D Effectiveness & Competency Mappings', category: 'HR Strategic', dashboard: 'hr' },
];
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-panel border border-panel-border p-4.5 rounded-2xl shadow-sm">
        <div className="space-y-1.5 flex-1 w-full min-w-0">
          ...
          <div className="flex items-center gap-2 group w-full">
            {isEditingName ? (
              <div className="flex items-center gap-2 w-full max-w-md">
                ...
                />
                <button onClick={saveName} className="bg-ink text-panel text-[10px] font-black uppercase px-3 py-1.5 rounded-lg cursor-pointer">
                  Save
                </button>
              </div>
            ) : (
          ...
          <p className="text-[11px] text-ink-soft font-light">
            Consolidated custom executive dashboard. Mix and match operational visuals, KPIs, and reports below.
          </p>
        </div>
      ...
      {customVisualIds.length > 0 ? (
            ...
            return (
              ...
              >
                ...
                <div className="p-4 bg-panel relative min-h-0">
                  <FocusedVisualWrapper visualId={vId} onDrillDown={onDrillDown} />
                </div>
              </div>
            );
          })}
        ...
        <div className="border-2 border-dashed border-panel-border rounded-2xl p-10 flex flex-col items-center text-center justify-center space-y-4 bg-panel/30">
          ...
          <div className="space-y-1 max-w-sm">
            <h3 className="text-sm font-bold text-ink uppercase tracking-wide">Your Dashboard is Empty</h3>
            <p className="text-xs text-ink-soft font-light">
              Build your customized strategy report. Add metrics cards, charts, pipelines, or roadmap items from any dashboard.
            </p>
          </div>
          <button
            onClick={() => setShowManager(true)}
          ...
          <div className="bg-panel border border-panel-border rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="bg-panel-2 px-5 py-4 border-b border-panel-border flex justify-between items-center shrink-0">
              <div>
                ...
                <p className="text-[10px] text-ink-soft font-light">
                  Select the visual blocks to render inside your custom executive report view.
                </p>
              </div>
              <button
            ...
            <div className="p-5 overflow-y-auto space-y-4.5">
              {['ceo', 'finance', 'gtm', 'hr'].map(db => {
                const dbName = 
                  db === 'ceo' ? 'CEO Exception Suite' :
                  db === 'finance' ? 'Sarah\'s Finance Suite' :
                  db === 'gtm' ? 'Chris\'s GTM Suite' : 'Sima\'s HR Suite';
                const items = AVAILABLE_VISUALS.filter(v => v.dashboard === db);
                      ...
                      {items.map(item => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className={`text-[8.5px] font-black px-2 py-1 rounded-md border shrink-0 transition-colors uppercase leading-none ${
                              isAdded
