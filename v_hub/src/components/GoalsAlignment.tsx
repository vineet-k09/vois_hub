			...
			<div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-2 pl-1">
					{req07.goals?.map((goal: any) => {
						...
						const getGoalTooltip = (g: any) => {
							return `${g.id}: ${g.text}. Current Status: ${g.status}. Linked context: ${g.context}. Click to drill down.`;
						};

						return (
							<Tooltip
								key={goal.id}
								content={getGoalTooltip(goal)}
								position="top">
								<div
									...
									<div className="flex items-center gap-2.5 min-w-0">
										...
										<div className="truncate">
											<p className="text-[11px] font-semibold text-ink truncate leading-snug">
												{goal.text}
											</p>
									...
									<div
										className={`px-2 py-0.5 rounded-full text-[9px] font-bold border shrink-0 leading-none ${textRagColor}`}>
										{goal.status}
									</div>
								</div>
							</Tooltip>
