...
export const FinanceDashboard: React.FC<FinanceDashboardProps> = ({
	...
	const { kpis, riskHeatmap, revenueAtRisk, costAtRisk, pqSummary } =
		financeData;
			...
			metrics: [
				{
					label: "Value",
					val: kpi.value,
					status: kpi.rag === "green" ? "Favorable" : "Action Set",
				},
				{
			...
			metrics: [
				{
					...
					val: `€${contract.value}M`,
					status: contract.risk === "High" ? "Critical" : "Watch",
				},
				{
					label: "Days to Expiry",
					val: `${contract.expiryDays} days`,
					status: "Urgent",
				},
				{
					label: "Auto-Renew Status",
					val: contract.autoRenew,
					status: "Pending",
				},
			],
		});
		...
		onDrillDown({
			...
			metrics: [
				...
				},
				{
					...
					val: `${project.completion}%`,
					status: "Progress",
				},
				{
					label: "Forecast Variance",
					val: `${project.burn - project.completion}pp gap`,
					status: project.rag === "red" ? "Critical" : "Watch",
				},
			],
		});
	};
				...
				<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 border-b border-panel-border pb-2.5 mb-3.5 pl-1">
					<div>
						...
						<p className="text-ink-soft text-[10px] italic font-light mt-0.5">
							Non-finance signals affecting Sarah's forecasts · heatmap / RAG
						</p>
					</div>
				</div>
					...
					<div className="border border-panel-border rounded-xl overflow-hidden bg-panel-2/30">
						<div className="bg-panel-2 border-b border-panel-border p-3 flex justify-between items-center">
							<span className="text-xs font-bold text-ink uppercase tracking-wider">
								Risk Heatmap — Signals by Portfolio
							</span>
						</div>
							...
							<table className="w-full text-left text-[11px] border-collapse min-w-125">
								<thead>
									<tr className="bg-panel-2 border-b border-panel-border">
										<th className="p-3 font-bold text-ink-soft uppercase tracking-wider text-[9px] w-1/4">
											Portfolio
										</th>
										<th className="p-3 font-bold text-ink-soft uppercase tracking-wider text-[9px] text-center w-1/5">
											HR / Headcount
										</th>
										<th className="p-3 font-bold text-ink-soft uppercase tracking-wider text-[9px] text-center w-1/5">
											Pipeline Demand
								...
								<tbody>
									{riskHeatmap.map((row) => (
										<tr
											key={row.portfolio}
											className="border-b border-panel-border/50 hover:bg-panel-2/20">
											<td className="p-3 border-r border-panel-border/30">
												<span className="font-bold text-ink block">
													{row.portfolio}
												</span>
												<span className="text-[10px] text-ink-soft italic font-light">
													{row.subtitle}
												</span>
											</td>
											{["hr", "pipeline", "operations", "contract"].map(
												(key) => {
													...
													return (
														<td
															...
															<span className="text-[9px] block mt-0.5 font-medium leading-tight">
																{cell.desc}
															</span>
														</td>
													);
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
						...
						<div className="border border-panel-border rounded-xl overflow-hidden bg-panel-2/30 flex flex-col justify-between">
							<div>
								<div className="bg-panel-2 border-b border-panel-border p-3 flex justify-between items-center">
									<span className="text-xs font-bold text-ink uppercase tracking-wider">
										Revenue at Risk — Lifecycles &lt; 6 Months
									</span>
								</div>
								<div className="overflow-x-auto">
									<table className="w-full text-left text-[11px] border-collapse min-w-87.5">
										<thead>
											<tr className="bg-panel-2 border-b border-panel-border">
												<th className="p-2.5 font-bold text-ink-soft uppercase tracking-wider text-[9px]">
													Contract
												</th>
												<th className="p-2.5 font-bold text-ink-soft uppercase tracking-wider text-[9px]">
													Customer
												</th>
												<th className="p-2.5 font-bold text-ink-soft uppercase tracking-wider text-[9px] text-right">
													Value
												</th>
												<th className="p-2.5 font-bold text-ink-soft uppercase tracking-wider text-[9px] text-center">
													Days
												</th>
												<th className="p-2.5 font-bold text-ink-soft uppercase tracking-wider text-[9px] text-center">
													Risk
											...
											{revenueAtRisk.map((c) => (
												<tr
													...
													<td className="p-2.5 text-center">
														<span
															className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase ${c.risk === "High" ? "bg-red-500/20 text-rose-500 border border-red-500/30" : "bg-amber-500/20 text-amber-500 border border-amber-500/30"}`}>
															{c.risk}
														</span>
													</td>
												</tr>
									...
									<div
										...
										<div className="flex items-center gap-3">
											<span className="text-ink-soft">
												Burn {p.burn}% vs Comp {p.completion}%
											</span>
											<span
												className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase ${p.rag === "green" ? "bg-emerald-500/15 text-emerald-500" : p.rag === "amber" ? "bg-amber-500/15 text-amber-500" : "bg-red-500/15 text-rose-500"}`}>
												{p.rag === "green"
													? "On Plan"
													: p.rag === "amber"
														? "Overspent"
	...
	return (
		<div className="space-y-4">
				...
				<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 border-b border-panel-border pb-2.5 mb-3.5 pl-1">
					<div>
						<h2 className="text-base font-bold text-ink uppercase tracking-wide">
							Financial Headline KPIs
						</h2>
						<p className="text-ink-soft text-[10px] italic font-light mt-0.5">
							{" "}
							Sarah's core financial control reporting closes
						</p>
					</div>
				</div>
						...
						return (
							<div
								...
								<div className="flex items-center justify-between w-full">
									...
									<span
										className={`w-1.5 h-1.5 rounded-full ${isGreen ? "bg-emerald-500" : isAmber ? "bg-amber-500" : "bg-red-500"}`}
									/>
								</div>
								<div className="my-2">
									<span
										className={`text-2xl  font-black tracking-tight ${ragColor}`}>
										{kpi.value}
									</span>
								</div>
			...
			<section
				id="finance-pq"
				className="bg-panel border border-panel-border rounded-2xl p-4.5 shadow-sm relative overflow-hidden">
				<div className="req-pin" title="Requirement #11">
					11
				</div>
				<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 border-b border-panel-border pb-2.5 mb-3.5 pl-1">
					<div>
						...
						<p className="text-ink-soft text-[10px] italic font-light mt-0.5">
							High-level on Hub · drill-through to PQ marketplace dashboard for
							L1/L2/L3 detail
						</p>
					</div>
				</div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-4 pl-1">
					...
					<div className="border border-panel-border rounded-xl p-3.5 bg-panel-2/30">
						...
						<div className="space-y-2">
							{pqSummary.kpis.map((k, idx) => (
								<div
									key={idx}
									className="flex justify-between items-center text-[11px] py-1 border-b border-panel-border/30 last:border-none">
									<span className="text-ink-soft font-medium">{k.label}</span>
									<span
										className={` font-bold text-sm ${k.rag === "green" ? "text-emerald-500" : "text-ink"}`}>
										{k.value}
									</span>
								</div>
							))}
					...
					<div className="border border-panel-border rounded-xl p-5 bg-linear-to-br from-panel-2 to-panel-2/40 flex flex-col justify-between text-center items-center">
						<div className="space-y-2">
							<h4 className="text-xs font-bold text-ink uppercase tracking-wider">
								PQ Marketplace Dashboard
							</h4>
							<p className="text-[10px] text-ink-soft font-light leading-relaxed">
								Full L1 · L2 · L3 service-type breakdown, market-specific views,
								and invoice-level detail. Context preserved on click-through.
							</p>
						</div>
						<button
							onClick={() =>
								alert(
									"Opening PQ Marketplace Dashboard... Context ( Sarah / Finance / FY27 YTD ) successfully transferred.",
								)
							}
						...
						<span className="text-[8px] text-muted-text mt-2 font-medium">
