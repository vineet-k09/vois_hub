						...
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-2.5">
							{cluster.kpis
								...
								.sort((a: any, b: any) => {
									...
									const isImportant = (kpi: any) =>
										importantLabels.some((label) => kpi.label.includes(label));
								...
								.map((kpi: any) => {
									...
									const isImportant = importantLabels.some((label) =>
										kpi.label.includes(label),
									);
									...
									if (isRed) {
										cardClass =
											"bg-rag-red/10 border-rag-red/20 hover:border-rag-red/30 hover:bg-rag-red/15 text-ink ring-1 ring-rag-red/5";
									} else if (isAmber) {
										cardClass =
											"bg-rag-amber/10 border-rag-amber/20 hover:border-rag-amber/30 hover:bg-rag-amber/15 text-ink ring-1 ring-rag-amber/5";
									} else if (isTbd) {
										cardClass =
											"bg-panel-2 border-panel-border hover:bg-panel hover:border-panel-border text-muted-text";
									} else {
										cardClass =
											"bg-panel border-panel-border hover:border-accent/40 hover:shadow-xs text-ink";
									}
									...
									// Trend text color
									const trendClass = isRed
										? "text-rag-red font-bold"
										: isAmber
											? "text-rag-amber font-bold"
											: "text-rag-green font-semibold";

									// Progress bar/text color
									const ragColorClass = isRed
										? "bg-rag-red"
										: isAmber
											? "bg-rag-amber"
											: "bg-rag-green";
									const ragTextClass = isRed
										? "text-rag-red"
										: isAmber
											? "text-rag-amber"
											: "text-rag-green";
									...
									return (
										<Tooltip
											key={kpi.label}
											content={getKPITooltip(kpi)}
											position="top">
											<div
												onClick={() => handleKPIClick(kpi, cluster.name)}
												className={`group p-3 rounded-xl border flex flex-col justify-between transition-all duration-200 cursor-pointer col-span-1 h-27 ${cardClass} relative overflow-hidden`}>
												...
												<div className="flex items-baseline justify-between gap-1.5 flex-wrap">
													<div className="flex items-baseline gap-1.5">
														<span
															className={`text-xl font-black tracking-tight leading-none ${valColor}`}>
															{kpi.value}
														</span>
														{kpi.trend && (
															<span
																className={`flex items-center gap-0.5 text-[9px] leading-none shrink-0 ${trendClass}`}>
																{kpi.trend.includes("▲") ? (
																	<TrendingUp size={9} />
																) : kpi.trend.includes("▼") ? (
																	<TrendingDown size={9} />
